...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  if (CGRectContainsPoint(self.bounds, touchLocation)) {
    ...
    if (HMSegmentedControlTypeText == _type)
    {
      if (( self.allowTapSelectedIndex || segment != self.selectedSegmentIndex ) && (segment < [self.sectionTitles count]))
      {
        // Check if we have to do anything with the touch event
        if (self.isTouchEnabled)
        {
          [self setSelectedSegmentIndex:segment
                               animated:self.shouldAnimateUserSelection notify:YES];
        }
      }
    }
    else if (HMSegmentedControlTypeImages == _type)
    {
      if (( self.allowTapSelectedIndex || segment != self.selectedSegmentIndex ) && (segment < [self.sectionImages count]))
      {
        if (self.isTouchEnabled)
        {
          [self setSelectedSegmentIndex:segment
